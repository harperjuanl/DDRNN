...
class DCGRUCell(RNNCell):
    ...
    def _gconv(self, inputs, state, output_size, bias_start=0.0):
        ...
        x = inputs_and_state     # 64,35,71
        x0 = tf.transpose(x, perm=[1, 2, 0])  # (num_nodes, total_arg_size, batch_size)  转置
        x0 = tf.reshape(x0, shape=[self._num_nodes, input_size * batch_size])   # 35 4544
        x = tf.expand_dims(x0, axis=0)         # 1,35,4544
        ...
        with tf.variable_scope(scope):           # GCN
            ...
            weights = tf.get_variable(
                ...
                initializer=tf.contrib.layers.xavier_initializer())
            x = tf.matmul(x, weights)  # (batch_size * self._num_nodes, output_size) 加入参数权重
